...
const Dashboard = () => {
  ...
  const Menus = [
    ...
    },
    {
      title: "Setting",
      path: "/",
      src: <FiSettings className="w-5 h-5" />,
      setting: true,
    },
    { title: "Go To Home", path: "/", src: <AiFillHome className="w-5 h-5" /> },
    {
  ...
  return (
    <div className="drawer drawer-mobile      rounded-3xl ">
      ...
      <div className="drawer-content custom-scroll">
        <label
          for="my-drawer-2"
          className="block mx-8 mt-3   drawer-button lg:hidden"
        >
        ...
        <ul className={`pt-6  ${!open ? "w-20" : "w-52"} p-5 bg-[#2366B5] `}>
          <BsArrowLeftCircle
            ...
            className={`absolute cursor-pointer
             -right-3  top-3 w-8 h-8 bg-black text-white    rounded-full ${
               !open && "rotate-180"
             }
             `}
          />
          <div className="mt-8">
            {Menus.map((menu, i) => (
              <Link to={menu?.path}>
                {" "}
                <li
                  className={`text-gray-300 ${!open && "justify-center"} ${
                    menu.gap ? "mt-9" : "mt-2"
                  } ${
                    menu.setting && "mt-60"
                  }  rounded-md text-sm flex items-center gap-x-4 p-2 cursor-pointer hover:bg-slate-500`}
                  key={i}
                >
                  {menu.src}{" "}
                  <span
                    className={`${
                      !open && "hidden"
                    } origin-left duration-200 text-md font-bold`}
                  >
                    {menu.title}
                  </span>
                </li>
            ...
            <li
              className={` text-white font-bold ${
                !open && "justify-center"
              }   rounded-md text-sm flex items-center gap-x-4 p-2 cursor-pointer hover:bg-red-700`}
            >
              <FiLogOut className="w-5 h-5" />
              <span
                className={`${
                  !open && "hidden"
                } origin-left duration-200 text-md font-semibold`}
              >
                Logout
              </span>
            </li>
